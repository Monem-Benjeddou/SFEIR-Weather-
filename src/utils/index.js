...
function addExampleLocations() {
    ...
    container.className = 'example-locations';
...
function updateWeatherInfo(weatherData, locationName) {
    ...
    if (locationElement) locationElement.textContent = locationName || 'Unknown Location';
    if (temperatureElement) temperatureElement.textContent = `${Math.round(weatherData.temperature_2m)}°C`;
    if (descriptionElement) descriptionElement.textContent = weatherData.weather_code ? getWeatherDescription(weatherData.weather_code) : 'N/A';
    if (humidityElement) humidityElement.textContent = `${Math.round(weatherData.relative_humidity_2m)}%`;
    if (windSpeedElement) windSpeedElement.textContent = `${Math.round(weatherData.wind_speed_10m)} km/h`;
}
...
function showSearchError(message) {
    ...
    if (locationElement) {
        ...
        locationElement.style.color = '#ff4444';
        setTimeout(() => {
            locationElement.style.color = ''; // Reset color after 3 seconds
        }, 3000);
    }
}
